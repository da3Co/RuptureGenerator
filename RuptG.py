...
class Rupture(object):
    ...
    def assingLocationsOrientations(self, strike, dip, rake, dll, dww, PoiR_I, PoiL):
        ...
        self.nll, self.nww = np.arange(0, self.L, self.dll), np.arange(0, self.W, self.dww)
        self.ade = [0, 0]
    ...
    def writeH5(self, filS):
        '''
        write with hdmf5 the rupture definition
        :param filS: Output file and extension
        :return:
        '''
        dtt= self.timi[1]-self.timi[0]
        with h5py.File(filS, 'w') as Sa:
            Sa.attrs.create('Vn', data=self.vnor)  # normal vector
            Sa.attrs.create('Vl', data=self.vlon)
            Sa.attrs.create('Vw', data=self.vwid)
            Sa.attrs.create('dip', data=np.deg2rad(self.dip))
            Sa.attrs.create('strike', data=np.deg2rad(self.strike))
            Sa.create_dataset('rake', data=[self.rakes])
            Sa.attrs.create('dl', data=self.dll)
            Sa.attrs.create('dw', data=self.dww)
            Sa.attrs.create('dt', data=dtt)

            Sa.attrs.create('m_vrvs', data=self.aVrr)
            Sa.attrs.create('loc', data=np.asarray([self.cll, self.cww, self.H]))
            Sa.attrs.create('locVr', data=self.locVr)
            Sa.attrs.create('umax', data=self.umax)
            Sa.attrs.create('m_tr', data=self.trp)
            Sa.create_dataset('Phyp', data=self.pHyp)

            Sa.create_dataset('Pos', data=self.Pos)
            Sa.create_dataset('Mom', data=[self.STFs])
            Sa.create_dataset('to', data=self.To)
            Sa.create_dataset('Tpeak', data=self.tac)
            Sa.create_dataset('trise', data=self.Trise)
            Sa.attrs.create('Mo', data=self.Mo)
            Sa.attrs.create('Mw', data=(np.log10(self.Mo) - 9.05) / 1.5)
            Sa.attrs.create('LW', data=self.nww.size*self.dww)
            Sa.attrs.create('LL', data=self.nll.size*self.dll)

            Sa.create_dataset('Vrup', data=self.Vr)
            Sa.create_dataset('Vss', data=self.Vs)
            Sa.create_dataset('Rho', data=self.Rho)
            #Sa.create_dataset('Vpp', data=self.Vp)
            #Sa.attrs.create('success', data=[ccS>0, ccTr>0, ccVr>0])
            #Sa.create_dataset('phase', data=[phaseS, phaseTo, phaseTr, phaseRa])
            #Sa.create_dataset('phaseSCV', data=phaseT)
            # np.argmin(tooF)
            Sa.attrs.create('Hypo', data=np.asarray(
                [self.Pos[0][self.hypo],self.Pos[1][self.hypo],self.Pos[2][self.hypo]]))
            Sa.attrs.create('Hy_dnLdnW', data=self.hypo)
            Sa.close()
